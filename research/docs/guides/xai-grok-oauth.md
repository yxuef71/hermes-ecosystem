...
Browser OAuth 2.0 PKCE (loopback callback)

Transport

xAI Responses API (`codex_responses`)
...
No (`XAI_API_KEY` is **not** used for this provider)

Subscription

[SuperGrok](https://x.ai/grok) or [X Premium+](https://x.com/i/premium_sign_up) — see note below
...
**Important:** the loopback listener still runs on the remote machine at `127.0.0.1:56121`. The xAI redirect needs to reach _that_ listener, so opening the URL on your laptop will fail (`Could not establish connection. We couldn't reach your app.`) unless you forward the port:
...
See [OAuth over SSH / Remote Hosts](/docs/guides/oauth-over-ssh) for the full step-by-step, including ProxyJump chains, mosh/tmux, and ControlMaster gotchas.
...
Override the default `https://api.x.ai/v1` endpoint (rarely needed).
...
Full walkthrough (jump boxes, mosh/tmux, port conflicts): [OAuth over SSH / Remote Hosts](/docs/guides/oauth-over-ssh).

### HTTP 403 after a successful login (tier / entitlement)

OAuth completed in the browser, tokens are saved, but inference or token refresh returns `HTTP 403` with a message similar to _"The caller does not have permission to execute the specified operation"_.
...
-   [OAuth over SSH / Remote Hosts](/docs/guides/oauth-over-ssh) — required reading if Hermes is on a different machine than your browser
-   [AI Providers reference](/docs/integrations/providers)
-   [Environment Variables](/docs/reference/environment-variables)
-   [Configuration](/docs/user-guide/configuration)
-   [Voice & TTS](/docs/user-guide/features/tts)
