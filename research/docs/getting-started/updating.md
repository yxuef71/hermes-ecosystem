...
If an earlier update pulled code but did not finish restarting the fleet, the next `hermes update` retries even when the checkout is already current. An empty process scan does not prove recovery: failed systemd units and installed launchd jobs may have no live PID. The pending restart marker is retained if supervisor discovery fails, a restart fails, or a requested service cannot be verified active. The update exits nonzero and reports the affected services; recover them with the printed commands and retry `hermes update`.
