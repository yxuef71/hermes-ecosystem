...
Recurring jobs are **at-most-once per occurrence, and every occurrence is accounted for**: it either runs (one execution row carrying its `scheduled_instant`), or its skip is logged with a reason. An occurrence is never dropped silently. The mechanics, in the order the due scan applies them (`cron/jobs.py::_evaluate_due_job`):
