...
**Source:** https://hermes-agent.nousresearch.com/docs/developer-guide/relay-shared-metrics
...
Hermes installs the platform-specific `nemo-relay` native wheel from the bounded `>=0.8.3,<0.9` dependency range. The published package is built from the [NVIDIA NeMo Relay repository](https://github.com/NVIDIA/NeMo-Relay). Unsupported platforms use the explicit no-op runtime described above rather than downloading a different implementation.
...
### A.2 Identity scope — the stable install\_id is transmitted as-is
...
-   It is generated locally (`uuid4`), contains no hardware, account, user, or machine-derived information, and identifies a _profile_, not a person.
...
Local identity, aggregates, and files reset; future sends use a new install\_id

Delete already-sent data

Not self-service — requires an operator acting on the S3 bucket
