...
An unpinned job stays on the provider/model it was created under, so changing your chat model never changes (or stops) your cron fleet. When you _do_ want scheduled jobs to move:
...
By default, agents launched _by_ the scheduler cannot use the `cronjob` tool — a scheduled job cannot create, edit, or remove other jobs. Opt in via `config.yaml`:
