...
**Source:** https://hermes-agent.nousresearch.com/docs/user-guide/features/kanban-multi-gateway
...
On the dispatch-owning gateway (typically the `default` profile), no change is needed. On every other profile gateway, add to `~/.hermes/config.yaml`:
...
dispatch\_in\_gateway

Opens subscribed board DBs?

Dispatcher

Notifier

default (confirmed dispatch-lock owner)
...
owned profiles + legacy unstamped subscriptions

writer, admin, coder, etc.

false

yes, when the profile has subscriptions

no

that gateway's owned profiles
