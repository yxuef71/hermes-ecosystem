...
-   `name` field present.
-   `description` field present, ≤ **1024 chars** (`MAX_DESCRIPTION_LENGTH`). **Long descriptions are truncated to 57 chars + "..." in the system prompt skill index** (`extract_skill_description` in `agent/skill_utils.py`); longer text is visible via `skills_list()` and `skill_view()`. Front-load the trigger phrase.
...
-   Description: ≤ 1024 chars (enforced). **Long descriptions render as the first 57 chars plus "..." in the system prompt skill index;** the rest is visible via `skills_list()` and `skill_view()`.
...
3.  **Description too generic or trigger buried past char 57.** The system prompt skill index truncates long descriptions at 57 chars. Peer descriptions start with "Use when ..." and complete the trigger class within that window.
