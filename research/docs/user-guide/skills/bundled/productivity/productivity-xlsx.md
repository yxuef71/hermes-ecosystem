...
`Excel`, `XLSX`, `Spreadsheets`, `Office`, `Productivity`
...
`markitdown file.xlsx` — `## SheetName` per sheet; reads `.xlsm` too. No cell coordinates, so don't plan edits from it. (`read_file` also auto-extracts .xlsx)

**Read** a model (formulas _and_ values)
...
2.  Spot-check 2–3 computed cells against expected values (`load_workbook(data_only=True)` _after_ recalc).
