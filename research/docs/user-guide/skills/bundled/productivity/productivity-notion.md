...
`Notion`, `Productivity`, `Notes`, `Database`, `API`, `CLI`, `Workers`
...
When asked to build a Worker, scaffold with `ntn workers new`, write the code in `src/index.ts`, set any secrets with `ntn workers env set`, and deploy. Notion's docs at [https://developers.notion.com/workers](https://developers.notion.com/workers) cover the full API surface.
