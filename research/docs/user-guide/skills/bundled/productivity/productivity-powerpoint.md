...
`PowerPoint`, `PPTX`, `Presentations`, `Office`, `Productivity`
...
Write a `pptxgenjs` script — see gotchas below

**Edit** an existing deck, or build from a template

unzip → edit `ppt/slides/slideN.xml` → zip

**Read** content

`markitdown deck.pptx` (one block per slide under `<!-- Slide number: N -->` markers); visual grid: `python scripts/thumbnail.py deck.pptx`
...
LibreOffice wrapper — bare `soffice` hangs in sandboxed environments
